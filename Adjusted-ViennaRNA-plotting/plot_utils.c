                                ...
                                unsigned int options)
{
  ...
  short     *ptable;
  char      *colorMatrix[6][4] = {
    { "0.0 1",  "0.0 0.6",  "0.0 0.2", "0.0 0.0" },   /* red    */
    { "0.16 1", "0.16 0.6", "0.16 0.2", "0.16 0.0" },   /* ochre  */
    { "0.32 1", "0.32 0.6", "0.32 0.2", "0.32 0.0" },   /* turquoise */
    { "0.48 1", "0.48 0.6", "0.48 0.2", "0.48 0.0" },   /* green  */
    { "0.65 1", "0.65 0.6", "0.65 0.2", "0.65 0.0" },   /* blue   */
    { "0.81 1", "0.81 0.6", "0.81 0.2", "0.81 0.0" }    /* violet */
  };
  vrna_md_t md;
  ...
  for (i = 1; i <= n; i++) {
    ...
    // count each iteration of the loop - thats how many sequences
    int seq_counter = 0;
    ...
    /*
    pfreq[0] / count from above loop
    Instead of pfreq[0], we use something from the fraction to get the shading (in colorMatrix[pairings - 1][pfreq[0]]))
    pairings is the color - bigger the #, the lower down the colorMatrix rows we go
    We delete pfreq[0] <= 2 condition so a color is always written out
    We need to add a 0 intensity column for colorMatrix (so we get output even if too many mismatches by fraction)
    */
    // Compute the fraction of mismatches 
    float fract_mismatches = ((float)pfreq[0]) / (float)seq_counter;
    if (pairings > 0) {
      // Now, need to determine which column the fraction should map to
      // For now, map using: 0.0/0.1/0.2/>0.2
      int col_index;
      if (fract_mismatches == 0.0){
        col_index = 0;
      } else if (fract_mismatches <= 0.1) {
        col_index = 1;
      } else if (fract_mismatches <= 0.2) {
        col_index = 2;
      } else {
        col_index = 3;
      }
      ...
      snprintf(pps, 64, "%d %d %s colorpair\n",
               i, j, colorMatrix[pairings - 1][col_index]);
      strcat(colorps, pps);    
    /*
